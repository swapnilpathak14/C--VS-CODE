...
long long binpow(long long a, long long b = modulo - 2, long long m = modulo)
{
...
void fourier(int *a, int n, int f = 0)
{
    ...
    if (f == 1)
        for (w = binpow(n, M - 2, M), i = 0; i < n; i++)
            a[i] = 1ll * a[i] * w % M;
}
...
int nCr(int n, int r)
{
    ...
    return (((inverse_factorial[r] * inverse_factorial[n - r]) % modulo) * factorial[n]) % modulo;
}
...
void prepare()
{
    ...
    for (int i = 2; i < 200005; i++)
    {
        ...
        book_keeping[i] = book_keeping[modulo % i] * (modulo - modulo / i) % modulo;
    }
    ...
    for (int i = 2; i < 200005; i++)
    {
        ...
        inverse_factorial[i] = (book_keeping[i] * inverse_factorial[i - 1]) % modulo;
    }
}
...
signed main()
{
    ...
    for (int ii = 0; ii < 30; ii++)
    {
        ...
        while (i <= n)
        {
            ...
            sol[i] = (sol[i] + ((1 << ii) * gotit[i])) % modulo;
            ;
